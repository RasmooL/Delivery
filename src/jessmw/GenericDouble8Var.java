...
public class GenericDouble8Var extends BeanSupport {
	...
	private double value[] = new double[8];
	
	
    /* Class Constructors */
	public GenericDouble8Var(String h, XmlFact f) throws CloneNotSupportedException {
	...
	public GenericDouble8Var(String h) throws CloneNotSupportedException {
		handlename = new String(h);
		handle     = SMRcomm.getByHandle(handlename);
		if ( ! fact.getStruct().getHost().getServerName().equals("mrc")) {
			if(!handle.addGeneric(fact.getStruct().getStructName() + "." + fact.getVariable(),this)) {
